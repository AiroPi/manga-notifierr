...
def setup_logger(
    ...
    levels: dict[str, int | None] | None = None,
) -> None:
    """Helper function to setup logger.

    Args:
        name: set the name of the logger you want to setup (usually root when None).
        log_file: set an optional file where logs are being printed.
        level: set the level of the logger setup.
        levels: set different levels for specific logger names (for logs from libraries). Set level to None to disable logging.
    """
    ...
    if levels:
        for name, custom_level in levels.items():
            logger = logging.getLogger(name)
